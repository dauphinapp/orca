...
struct CampusMapView: View {
  ...
  @State private var shouldRestoreOverviewOnDismiss = true

  var body: some View {
    Map(position: $position) {
      ForEach(CampusLocations.all) { location in
        let isSelected = selectedLocation?.id == location.id

        Annotation(location.name, coordinate: location.coordinate, anchor: .bottom) {
          Button {
            shouldRestoreOverviewOnDismiss = true
          ...
          .accessibilityIdentifier("map.annotation.\(location.code)")
        }
      }
    }
  ...
  private func selectLocation(_ location: CampusLocation) {
  ...
  private func focusedPosition(for location: CampusLocation) -> MapCameraPosition {
    .camera(.init(centerCoordinate: location.coordinate, distance: 500, heading: 0, pitch: 35))
  }

  private func restoreOverviewCamera() {
    if shouldRestoreOverviewOnDismiss, let overviewPosition {
      withAnimation(.easeInOut(duration: 0.2)) {
        position = overviewPosition
      }
    }
    shouldRestoreOverviewOnDismiss = true
    overviewPosition = nil
  }

  private func returnToLocationList() {
    shouldRestoreOverviewOnDismiss = false
